...
export default class Utils {
  ...
  static getPartners = async (_partnerIds) => {
    try {
      return await Utils.contactSystem(
        "get",
        "/system/partners?ids=" + _partnerIds
      );
    } catch (_e) {
      throw _e;
    }
  ...
  static checkPartner = async (_email, _phone) => {
    try {
      return await Utils.contactSystem(
        "get",
        "/system/partners/lookup?email=" + _email + "&mobile=" + _phone
      );
    } catch (_e) {
      throw _e;
    }
  ...
  static checkCustomerAssociation = async (
    _vendorId,
    _customerId,
    _associate = "NO"
  ) => {
  ...
  static handleAxiosError = (_e, _res) => {
    Log.log(_e.stack, Log.TYPE.TRACE);
    _res.status(500);
    _res.setHeader("content-type", "application/json");
    _res.json({ errors: true, message: _e?.response?.data || _e.message });
  };
