...
import { getSendContinueVendorTemplate, getTicketTemplate } from "../helpers/whatsapp-template-helper.js";
/**
...
const getAllTickets = async (_req) => {
  try {
    const search = _req.query.search ? _req.query.search : null;
    const pageSize = parseInt(_req.query.page_size) || parseInt(process.env.PAGE_SIZE)
    const getTicketsWithNoEntries = _req.query.get_tickets_no_entries === "true" ? true : false
    const page = search ? 0 : parseInt(_req.query.page) || 0;
    const skip = page * pageSize;
    const limit = pageSize;
    let checkTestersMobileNumber = false
    const user = _req.headers.user
    const eventModel = await EM.getModel("eventsDB", "events");
    const ticketOrders = await EM.getModel("ticketsDB", "ticket_orders")
    ...
    const testersMobileNumbers = ["7899020430", "8147113798", "8861278272", "8088020619", "9740452978", "7204357841", "9353478074", "9741536152", "7483626790", "9964533375", "8970063505", "9972538979"]
    const findConfig = {
    ...
    if (vendor) {
      ...
      if (vendorList?.length > 0) {
        ...
        if (vendorList.includes("656c73b0cb27bc8c6241e70c")) {
          checkTestersMobileNumber = false
          delete query.customer_mobile
        } 
      }
    }
    // online offline
    ...
    for (let i = 0; i < ticketOrdersCount?.length; i++) {
      const vendorData = await getPartnerByPartnerId({ id: ticketOrdersCount[i]?.vendor })
      // calculate vendor ticket counts
      if (ticketOrdersCount[i].ticket_status) {
        if (!vendorTicketCounts[ticketOrdersCount[i].vendor]) {
          vendorTicketCounts[ticketOrdersCount[i].vendor] = { "vendor_id": ticketOrdersCount[i]?.vendor, "vendor_name": vendorData?.name, "total_ticket_sales": parseInt(ticketOrdersCount[i].total_price), "total_platform_fee": parseInt(ticketOrdersCount[i].platform_fee), "total_gst_fee": parseInt(ticketOrdersCount[i].gst_fee), "online_tickets_count": ticketOrdersCount[i]?.user_id ? 0 : 1, "offline_tickets_count": ticketOrdersCount[i]?.user_id ? 1 : 0, "offline_head_count": 0, "online_head_count": 0 }
        } else {
          vendorTicketCounts[ticketOrdersCount[i].vendor]["total_ticket_sales"] += ticketOrdersCount[i].total_price
          vendorTicketCounts[ticketOrdersCount[i].vendor]["total_platform_fee"] += parseInt(ticketOrdersCount[i].platform_fee)
          vendorTicketCounts[ticketOrdersCount[i].vendor]["total_gst_fee"] += parseInt(ticketOrdersCount[i].gst_fee)
    ...
    for (let i = 0; i < ticketData.length; i++) {
      ...
      const ticketFilteredData = ticketData[i].tickets.map((ticket) => {
        const packageParsedData = JSON.parse(ticket.package_data)
        totalEntries += (parseInt(ticket?.quantity) - parseInt(ticket?.balance)) * packageParsedData?.package_map?.package?.pax
        return {
      ...
      if (getTicketsWithNoEntries && totalEntries !== 0) {
        _count -= 1
        continue
      }
    ...
    return {
      ...
      vendor_counts: Object.values(vendorTicketCounts)
    };
  } catch (err) {
    throw err
  }
...
const getTicketCounts = async (_req) => {
  try {
    ...
    allTicketOrders?.forEach(ticketOrder => {
      ...
      payload.platform_fee_total += parseInt(ticketOrder?.platform_fee)
      payload.gst_fee_total += parseInt(ticketOrder?.gst_fee)
...
const getAllCustomerData = async (_req) => {
  try {
    ...
    payload.sort((a, b) => new Date(b.customer_data.createdAt) - new Date(a.customer_data.createdAt))
    const testersMobileNumbers = ["7899020430", "8147113798", "8861278272", "8088020619", "9740452978", "7204357841", "9353478074", "9741536152", "9972538979", "7483626790", "9964533375", "8970063505"]
    payload = payload.filter(data => (!testersMobileNumbers.includes(data?.customer_data?.mobile)) && (data?.customer_mapped_partner?._id !== "656c73b0cb27bc8c6241e70c"))
...
// super admin api to send whats app message to users
const sendContinueBooking = async (_req) => {
  const temp_cache_for_customer_ids = []
  try {
    const { event_id, customer_id } = _req.body
    const eventModel = await EM.getModel("eventsDB", "events");
    const user = _req.headers.user
    if (!user) {
      throw new Error("Access denied user not found")
    ...
    const currentUser = await getUserInfo({ _id: user })
    // add only one super admin name
    if ((!currentUser || currentUser.role.handle !== "system")) {
      throw new Error("Access denied, only system users can access this API")
    }
    if (currentUser?.partner?.email !== "ganesh@ticketsque.com") {
      throw new Error("Access denied you do not have permission to use this feature")
    }
    if (!(event_id || customer_id)) {
      throw new Error("Invalid request, reqired fields not in request body")
    }
    const eventData = await eventModel?.findById({ _id: event_id })
    if (!eventData) {
      throw new Error("Invalid request, event data not found")
    }
    const customerData = await getPartnerByPartnerId({ id: customer_id })
    if (!customerData) {
      throw new Error("Invalid request, customer data not found")
    }
    const vendorData = await getPartnerByPartnerId({ id: eventData?.vendor })
    if (!vendorData) {
      throw new Error("Invalid request, vendor data not found")
    }
    const event_data = {
      eventName: eventData?.name,
      eventImage: (await getSignedLink({ key: eventData.images[0] })).image_link,
      eventHandle: eventData?.handle
    }
    const vendor_data = {
    ...
    const customer_data = {
      ...
      customerNumber: customerData?.mobile
    }
    const template = getTicketTemplate(event_data, vendor_data, customer_data)
    const config = {
      method: "post",
    ...
    //no await for whatsApp message sending
    const whatsappCall = axios.request(config);
    whatsappCall
      .catch(err => {
...
// super admin api to send whats app message to users
const sendContinueVendorMessage = async (_req) => {
  const temp_cache_for_customer_ids = []
  try {
    const { customer_id, vendor_id } = _req.body
    const user = _req.headers.user
    if (!user) {
      throw new Error("Access denied user not found")
    }
    ...
    // if(currentUser?.partner?.email !== "ganesh@ticketsque.com" && currentUser?.partner?.email !== "vishwa.prasad@ticketsque.com") {
    //   throw new Error("Access denied you do not have permission to use this feature")
    // }
    ...
    const vendorData = await getPartnerByPartnerId({ id: vendor_id })
    const vendorShortName = await getVendorShortName({ vendorId: vendor_id })
    if (!vendorData) {
      throw new Error("Invalid request, vendor data not found")
    }
    const vendor_data = {
    ...
    const template = getSendContinueVendorTemplate(customer_data, vendor_data)
    const config = {
      method: "post",
      url: process.env.WHATSAPP_URL,
      data: { ...template },
